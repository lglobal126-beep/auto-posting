...
def generate_shorts_script(
    *,
    ...
    prompt = f"""
【글자 수 절대 제한: 공백 포함 {char_max}자 이하. 이 제한을 초과하면 실패로 처리됩니다.】

당신은 여단오 채널 스타일의 숏폼 음식 리뷰 크리에이터입니다.
아래 음식점의 숏츠 나레이션 대본을 작성해주세요.
...
여단오 채널의 음식 리뷰 방식을 따릅니다:
• "쌰갈" — 음식이 기대 이상으로 맛있을 때 터져나오는 감탄. "어떻게 이렇게 맛있어 쌰갈", "이거 실화야 쌰갈"
• 친구한테 카톡 보내듯 말함 — "야 여기 진짜야", "나 여기 또 왔어"
• 짧은 문장, 자연스러운 끊김 — 호흡대로 끊어 읽히도록
• 과장 없이 진짜 리액션, 음식 식감 의성어 — "바삭", "쫙", "촉촉"

━━━ 장면 구성 ({dur}초 분량) ━━━

① 훅 (0~{t1}초): 엄지 멈추게 할 한 마디
② 가게 소개 ({t1}~{t2}초): 지역·분위기 짧게
③ 음식 묘사 ({t2}~{t3}초): 색감·식감·맛 감각적으로
④ 한 입 리액션 ({t3}~{t4}초): 진짜 감동, 쌰갈 한 번
⑤ CTA ({t4}~{dur}초): "저장해두고 꼭 와봐" 류

━━━ 필수 규칙 ━━━
• 이모지, 마크다운, 특수문자 절대 금지 (TTS 낭독용)
• 공백 포함 총 {char_min}자 이상 {char_max}자 이하 — 반드시 준수
• {char_max}자 초과 시 즉시 삭제하여 맞출 것

━━━ 출력 형식 ━━━

[SHORTS_SCRIPT]
(대본만 작성, 장면 번호나 설명 없이 순수 나레이션 텍스트만)
[/SHORTS_SCRIPT]
""".strip()
