...
def generate_shorts_script(
    *,
    ...
    # TTS rate=+50% 적용 시 실측 발화 속도: 약 4.5자/초
    # 따라서 목표 오디오 길이(=video_duration)에 맞는 글자 수 계산
    base_dur = video_duration or 20
    dur = base_dur  # 장면 타임스탬프는 영상 길이 기준
    char_min = max(30, int(base_dur * 3.8))
    char_max = int(base_dur * 4.5)
    # 장면 타임스탬프 비율
    t1 = min(3, int(dur * 0.12))
    t2 = int(dur * 0.30)
    t3 = int(dur * 0.65)
    t4 = int(dur * 0.85)

    prompt = f"""
당신은 숏폼 음식 크리에이터입니다. 아래 음식점의 {dur}초짜리 숏츠 나레이션 대본을 써주세요.
...
스타일 (여단오 채널 참고):
- 훅: "잠깐, 이거 그냥 지나치면 후회함" 같이 엄지 멈추게 하는 한 마디로 시작
- 친구한테 카톡 보내듯 — "야 여기 진짜야", "나 여기 또 옴"
- 짧게 끊어 리듬감 있게 — 한 문장에 10자 내외
- 식감 의성어 적극 활용 — "바삭", "쫙 찢어지는", "촉촉"
- "쌰갈" 한 번 — 감동 절정 순간에만
- 마지막엔 저장 유도 — "저장해두고 꼭 와봐"

장면 흐름 ({dur}초):
0~{t1}초: 훅 (엄지 멈추게 할 한 마디)
{t1}~{t2}초: 가게/지역 짧게 소개
{t2}~{t3}초: 음식 비주얼·식감·맛 묘사
{t3}~{t4}초: 한 입 리액션 + 쌰갈
{t4}~{dur}초: 저장 유도 CTA

규칙 (위반 시 실패):
1. 이모지, 마크다운, 특수문자 절대 금지 (TTS 낭독용)
2. 공백 포함 {char_min}자 이상 {char_max}자 이하 — 절대 초과 금지
3. 대본만 출력, 장면 번호나 설명 없이

[SHORTS_SCRIPT]
(순수 나레이션 텍스트만, {char_max}자 이하)
[/SHORTS_SCRIPT]
""".strip()
    ...
    body = {
        ...
        "generationConfig": {
            "temperature": 0.9,
            "maxOutputTokens": 300,  # 글자 수 초과 방지
        },
    }
